...
PLATFORM="${MERGEFORGE_PLATFORM:-linux/amd64}"
echo "Building MergeForge images for ${PLATFORM}..."

echo "Building MergeForge web image..."
docker build --platform "${PLATFORM}" -t mergeforge-web:0.1.0 web
...
docker build --platform "${PLATFORM}" -t mergeforge-dogecoin:1.14.9 dogecoin
