# MergeForge 0.1.4 Physical Acceptance Test

1. Build both local images.
2. Install/start only through Umbrel lifecycle controls.
3. Confirm dashboard `/api/health` returns version 0.1.4.
4. Confirm host listens on 3333 and does not expose c2pool API 8080 directly.
5. Wait until c2pool reports backend readiness.
6. Configure a Scrypt miner to `stratum+tcp://UMBREL_IP:3333`, username = `LTC_ADDRESS,DOGE_ADDRESS.ScryptMiner`, password = `x`.
